#!/usr/bin/env bash
# Topic hygiene: idle/dead topic report & cleanup, plus topics with no active consumers.
#
# Default: print age-bucket table.
# --list-no-consumers: topics with no live consumer assignment.
# --delete: idle longer than --idle-days, optionally requiring no active consumers.
#
# Usage:
#   ./topic_hygiene.sh -c ../kafka_realtime_check/config/clusters/devkafka.env -u USER
#   ./topic_hygiene.sh -c … -u USER --list-no-consumers
#   ./topic_hygiene.sh -c … -u USER --idle-days 180 --with-consumers
#   ./topic_hygiene.sh -c … -u USER -y --idle-days 180 --delete --require-no-consumers --apply
#   ./topic_hygiene.sh -c … -u USER -y --idle-days 180 --delete --allow-with-consumers --apply
#
set -euo pipefail
...
ACTION="report"   # report | delete | list-no-consumers
...
Topic hygiene v${SCRIPT_VERSION} (idle report / no-consumer list / cleanup)

Actions:
  --report                 Age-bucket table + TSV (default)
  --list-no-consumers      List topics with no active consumer assignment
...
Note: "active consumer" = a live group member (CONSUMER-ID) assigned to the topic.
      Empty groups / committed offsets alone do not count as attached.
...
Examples:
  ...
  $(basename "$0") -c … -u USER --list-no-consumers
  $(basename "$0") -c … -u USER --list-no-consumers --pattern '^charisma\\.oms\\.'
...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --list-no-consumers|--no-consumers) ACTION="list-no-consumers"; shift ;;
...
_run_list_no_consumers() {
  ...
  emit "Cluster topics: ${total}  filter-dropped: ${filtered}  selected: ${#topics[@]}"

  _ACTIVE_TOPICS_FILE="$(mktemp "${TMPDIR:-/tmp}/idle-active-topics.XXXXXX")"
  : >"$_ACTIVE_TOPICS_FILE"
  _collect_active_consumer_topics

  local no_c=0 with_c=0
  {
    printf 'topic\thas_active_consumer\n'
    local t has
    for t in "${topics[@]}"; do
      has=0
      if [[ -s "$_ACTIVE_TOPICS_FILE" ]] && grep -Fxq "$t" "$_ACTIVE_TOPICS_FILE" 2>/dev/null; then
        has=1
        with_c=$((with_c + 1))
      else
        no_c=$((no_c + 1))
  ...
  emit ""
  emit "With active consumer: ${with_c}"
  emit "No active consumer:   ${no_c}"
  emit "Wrote ${TSV_OUT}"

  section "Topics with no active consumer (${no_c})"
  if (( no_c == 0 )); then
    emit "${C_GREEN}None — every selected topic has a live consumer assignment.${C_RESET}"
  else
    awk -F'\t' 'NR>1 && $2==0 { print "  " $1 }' "$TSV_OUT"
  fi
  rm -f "$_ACTIVE_TOPICS_FILE"
}
...
main() {
  ...
  if [[ "$ACTION" != "list-no-consumers" ]]; then
  ...
  if [[ -z "$TSV_OUT" ]]; then
    local slug
    slug="$(basename "${CONFIG_FILE:-cluster}" .env)"
    slug="${slug//[^A-Za-z0-9._-]/_}"
    slug="${slug##_}"; slug="${slug%%_}"
    [[ -n "$slug" ]] || slug="cluster"
    if [[ "$ACTION" == "list-no-consumers" ]]; then
      TSV_OUT="${REPORT_DIR}/${slug}-no-consumers.tsv"
    else
      TSV_OUT="${REPORT_DIR}/${slug}-idle-topics.tsv"
    fi
  fi

  emit "${C_BOLD}Topic hygiene v${SCRIPT_VERSION}${C_RESET} — ${CLUSTER_NAME:-cluster}"
  emit "Config: ${CONFIG_FILE}"
  if [[ "$ACTION" == "list-no-consumers" ]]; then
    emit "Action: ${ACTION}  (topics with no live consumer assignment)"
  else
