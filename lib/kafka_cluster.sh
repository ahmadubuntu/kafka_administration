...
_kafka_remote_tool() {
  # Run kafka CLI on a broker host via SSH when checker lacks binaries
  local host="$1"; shift
  local tool="$1"; shift
  local bootstrap="${KAFKA_CONNECT_BOOTSTRAP:-${KAFKA_BOOTSTRAP:-}}"
  local conf="${KAFKA_COMMAND_CONFIG:-/opt/kafka/config/admin.properties}"
  local bin="${KAFKA_BIN:-/opt/kafka/bin}"
  local remote_cmd
...
_kafka_run() {
  local tool="$1"; shift
  if [[ -x "${KAFKA_BIN:-/opt/kafka/bin}/${tool}" ]]; then
    _kafka_tool "$tool" "$@"
    return $?
  fi
  # Fallback: first SSH-OK broker
  local brokers=() h
...
check_kafka_services() {
  ...
  local brokers=() unit="${KAFKA_SYSTEMD_UNIT:-kafka}"
  csv_to_array brokers "${BROKER_HOSTS:-}"
  local h
  for h in "${brokers[@]}"; do
    if ! ssh_host_is_ok "$h"; then
